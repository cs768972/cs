...
public class UserController {
	 ...
	 * 
	 * @ResponseBody public String login(String userName, String userPass,
	 * HttpServletRequest request) {
	 * 
	 * Users user = userService.login(userName,MD5Utils.passToMD5(userPass)); if
	 * (user != null){ HttpSession session =request.getSession();
	 * session.setAttribute("user", user); return "success"; } return "fail"; }
	 */
	  ...
	  @ResponseBody public String login(String userName,String userPass, Model
	  model, HttpSession session) { // 通过账号和密码查询用户
	  
	
	  Users users = userService.login(userName,MD5Utils.passToMD5(userPass));
	  if(users != null){
		  // 将用户对象添加到Session 
		  session.setAttribute("user", users); //  跳转到主页面 
	// return "customer"; return "redirect:tea/list.action"; 
		  return "success"; 
		  } 
		  model.addAttribute("msg", "账号或密码错误，请重新输入！"); // 返回到登录页面
		  return "login"; 
	  }
